...
import { updateAttr, updateChildren } from './helper.js'
const genSize = (w, d) =>
{
 ...
 * type = SVG HTML태그.
 */
const genElement = (type) => ( tag, attr = {}, children = []) =>
{
    const el = type === 'svg'
...
const genSvgFromList = (list, w, d, i, v) =>
{
    ...
    for (const [name, info] of (Object.values(list)))
    {
            ...
            if (Array.isArray(info.id))
            {
                for (const id of info.id)
                {
                    if (Array.isArray(info.attr))
                    {
                        ...
                        {
                            temp = S(w, d, i, v)(attr, info.type, id)
                    ...
                    {
                        temp = S(w, d, i, v)(info.attr, info.type, id)
                        updateAttr(temp, { id: id, name: info.name })
                        createdSVG[id] = temp
                    }
        ...
        temp = S(w, d, i, v)(info.attr, info.type)
...
const genFragment = (children) => updateChildren(document.createDocumentFragment(),children)


export { genAttr, genSize, genPath, genElement, genSvgFromList, genSvgList, genFragment }