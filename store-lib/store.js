...
const Store = (msg, sub) => {
    ...
    const ack = ({ topics }) => {
        topics.forEach((topic) =>
            _store[topic].subs.forEach((s) => s(_store[topic].data))
        )
    }
    ...
    const setData = (msgs, sub) => {
        const topics = msgs.reduce((acc, cur) => {
            ...
            const temp = {
                [cur.topic]: {
                    data: prevStore
                        ? { ...prevStore['data'], ...cur.data }
                        : { [Symbol.toStringTag]: cur.topic, ...cur.data },
    ...
    const pubSubPipe = (msgs, subs) => [
        pipe(setData(msgs, subs), subscribe, ack),
    ]
